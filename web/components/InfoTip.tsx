...
export function InfoTip({ text }: { text: string }) {
  ...
  return (
    <span className="relative inline-flex">
      ...
      {open && (
        <span className="mi-panel absolute bottom-full left-1/2 z-50 mb-2 w-60 max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-lg border border-[var(--color-line)] bg-[var(--color-panel-2)] p-3 text-left text-xs font-normal leading-relaxed text-[var(--color-muted)] shadow-2xl">
          {text}
        </span>
      )}
    </span>
